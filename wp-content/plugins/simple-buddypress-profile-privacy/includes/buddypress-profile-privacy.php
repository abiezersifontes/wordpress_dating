...
class SBPP04_Profile_Privacy extends BP_Component {
    ...
    public function __construct() {
        ...
        // Unique component ID
            ...
            __( 'BP Profile Privacy', 'buddypress-profile-privacy' )
        );
    }
    ...
    public function setup_nav( $main_nav = array(), $sub_nav = array() ) {
        $main_nav = array(
            'name'                => __( 'Private', 'buddypress-profile-privacy' ),
            'slug'                => $this->slug,
            'screen_function' => array( $this, 'screen_function_main' ),
            'default_subnav_slug' => 'hidden',
            'position'            => 10000,
        );

        $sub_nav[] = array(
            'name'            => __( 'Friends Only', 'buddypress-profile-privacy' ),
            'slug'            => 'hidden',
    ...
    public function main_content()
    {
		...
		if( is_user_logged_in() ) {
			echo " Use the button below to send a friend request to " . bp_core_get_user_displayname( bp_displayed_user_id() );
...
function sbpp04_profile_settings_nav() {
	...
	bp_core_new_subnav_item( array(
		'name' => __( 'Privacy Settings', 'buddypress' ),
...
function sbpp04_privacy_screen_content() {
	...
	?>
		<form action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" method="post">
			<p><label for="can_view_proile">Who can view your profile?</label>
			<select name="can_view_profile">
				<option value="<?php echo esc_attr( SBPP04_VIEW_EVERYONE ); ?>"<?php if( $curr_privacy == SBPP04_VIEW_EVERYONE ) echo ' selected="selected"' ?>>Everyone</option>
				<option value="<?php echo esc_attr( SBPP04_VIEW_LOGGED_IN ); ?>"<?php if( $curr_privacy == SBPP04_VIEW_LOGGED_IN ) echo ' selected="selected"' ?>>Only Logged In Users</option>
                <?php if( SBPP04_FRIENDS_ACTIVE ): ?>
				    <option value="<?php echo esc_attr( SBPP04_VIEW_FRIENDS ); ?>"<?php if( $curr_privacy == SBPP04_VIEW_FRIENDS ) echo ' selected="selected"' ?>>Only Friends</option>
                <?php endif; ?>
			</select></p>
			<?php if( bp_get_option( SBPP04_ADMIN_HIDE_DIRECTORY_KEY ) == 1 ) : ?>
			<label for="hide_directory">Hide My Profile from Member Directory</label>
			<p><select name="hide_directory">
				<option value="Yes"<?php if( $hide_dir == "Yes" ) echo ' selected="selected"' ?>>Yes</option>
				<option value="No"<?php if( $hide_dir == "No" ) echo ' selected="selected"' ?>>No</option>
			</select></p>
			<?php endif; ?>
			...
			case SBPP04_VIEW_FRIENDS:
                ...
                if( SBPP04_FRIENDS_ACTIVE ){
                    ...
                    if( $is_friend != 'is_friend' && !bp_is_my_profile() && !bp_is_current_component( 'bpp' ) ) {
                        wp_redirect( bp_displayed_user_domain() . 'profile-privacy/hidden' );
                        exit();
                    }else{
...
/*
 * Filter out the members who want to be hidden from the directory.
 */
function sbpp04_filter_directory( $retval ){
	if( bp_is_members_directory() && bp_get_option( SBPP04_ADMIN_HIDE_DIRECTORY_KEY ) == 1 ){
		$exclude_array = array();
		...
		if( !empty( $exclude_query->results ) ){
			foreach( $exclude_query->results as $user ){
				$exclude_array[] = $user->ID;
			}
			$retval['exclude'] = $exclude_array;
		}
	}
	return $retval;
}
add_filter( 'bp_after_has_members_parse_args', 'sbpp04_filter_directory' );
